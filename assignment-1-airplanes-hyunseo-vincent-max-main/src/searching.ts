 ...
 * Ω(1)
 */
function compareExactMatch(str1: string, str2: string): boolean {
 ...
 */
function findStartingIndex(arr: string[][], prefix: string): number {
  let start: number = 0;
 ...
 * Improved & fixed (from chat-gpt)
 */
function findEndingIndex(arr: string[][], prefix: string): number {
  let start: number = 0;
  let end: number = arr.length - 1;
